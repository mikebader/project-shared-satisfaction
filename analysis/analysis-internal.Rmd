...
## Distribution of respondents across neighborhoods

### Median satisfaction level in tracts
...
### Count of respondents across neighborhoods
...
There of the `r nrow(wncnt)` neighborhoods with two or more respondents, `r all_pres` neighborhoods (`r round(100 * all_pres / nrow(wncnt), 1)` percent) contain respondents who identify across all four racial groups. Table \@ref(tab:nhood-desc) contains descriptive statistics of the distribution of the racial identities of respondents across neighborhoods and Table \@ref(tab:nhood-only) contains the number of *respondents* who live in neighborhoods where all other respondents share the same racial identity. 

```{r wn-nhood-race-summary}
...
wn_mfx_plt
ggsave("images/marginal_effects_within.pdf", plot = wn_mfx_plt,
       width=6.75, units='in')
```

## Partial Predicted Probabilities
