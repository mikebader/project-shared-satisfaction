...
The following report predicted probabilities of satisfaction for respondents sampled from multiracial neighborhoods compared to residents drawn from all neighborhoods. 

```{r supp-combined-lincom}
...
psat_api_quad <- plogis(sum(coef(MIcombine_aic(m3C))[
    c('(Intercept)', 'nhdsmpquad', 'raceethasian', 'nhdsmpquad:raceethasian')]))
psat_api_all <- plogis(sum(coef(MIcombine_aic(m3C))[
    c('(Intercept)', 'raceethasian')]))
pct <- function(x) {round(100 * x, 1)}
```

Table: Predicted percent of residents satisfied in types of sampled neighborhoods by race {#tab:psat}

Race                     All         Multiracial
--------- ------------------ -------------------
Asian      pct(psat_api_all)  pct(psat_api_quad)
Black      pct(psat_nhb_all)  pct(psat_nhb_quad)
Latinx     pct(psat_hsp_all)  pct(psat_hsp_quad)
White      pct(psat_nhw_all)  pct(psat_nhw_quad)
