...
for (nhd in quad.nhoods) {
    ...
    colnames(m$variance) <- varnames
    rownames(m$variance) <- varnames
}

cons_labels <- c(regression_labels[1:(length(regression_labels)-3)], 
                 'Lives in community')
cons_tbl <- report_models(Herndon=m3_con$herndon, Germantown=m3_con$germantown,
                          Wheaton=m3_con$wheaton, reglabels=cons_labels,
