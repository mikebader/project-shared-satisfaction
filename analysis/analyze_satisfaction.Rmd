...
```{r extremely-satisfied-analysis}
...
```{r plot-consider-descriptives}
consider_means$comm <- ordered(consider_means$comm,
                               levels=comm_names)
consider_means$race <- ordered(consider_means$race,
                               levels=c('api', 'black', 'latino', 'white',
                                        'overall'))
consider_plt_df <- consider_means[consider_means$race=='overall',
                                  c('comm','prop')] %>%
                   rename(overall = prop) %>%
                   mutate(pos=c(0,2,8,10,12,16,18,24,28,30,32)) %>%
                   right_join(consider_means[consider_means$race!='overall',],
                              by='comm')
dodge_width <- 1
ggplot(consider_plt_df, aes(x=pos, y=prop, shape=race)) + 
    geom_errorbar(aes(ymin=overall,ymax=prop), 
                  position=position_dodge(dodge_width), width=0) +
    geom_point(size=2, position=position_dodge(dodge_width), fill='white') +
    annotate("text", x=counties$x, y=.3, label=counties$name, size=3, hjust=1) +
    scale_x_continuous(breaks=c(0,2,8,10,12,16,18,24,28,30,32),
                       labels=levels(consider_plt_df$comm)) +
    scale_y_continuous(breaks=seq(0,.25,0.05), limits=c(0,.30)) +
    labs(
        x='Community', y='Proportion', 
        title='Proportion willing to consider community by race' 
    ) + 
    coord_flip() +
...
### Separate plot for consider by each race
# colors <- c('darkslategray4', 'tomato1')
# for(grp in c('overall', 'white','black','latino','api')){
#     color_scale <- if(grp=='overall') rev(colors) else colors
#     ggplot(subset(consider_means, race==grp|race=='overall'), 
#            aes(x=comm, y=prop, color=race)) + 
#         geom_point(size=1.5) +
#         scale_color_manual(values=color_scale) +
#         coord_flip() + ylim(c(0,.25)) +
#         labs(
#             x='Community', y='Proportion', 
#             title='Proportion willing to consider neighborhood' 
#         )
#     ggsave(sprintf('images/consider_%s.png', grp), 
#            width=8, height=4.5, units='in')
# }

```

```{r plot-not-consider-descriptives}
nconsider_means$comm <- ordered(nconsider_means$comm,
                               levels=comm_names)
nconsider_means$race <- ordered(nconsider_means$race,
                               levels=c('api', 'black', 'latino', 'white',
                                        'overall'))
nconsider_plt_df <- nconsider_means[nconsider_means$race=='overall',
                                  c('comm','prop')] %>%
                   rename(overall = prop) %>%
                   mutate(pos=c(0,2,8,10,12,16,18,24,28,30,32)) %>%
                   right_join(nconsider_means[nconsider_means$race!='overall',],
                              by='comm')

ggplot(nconsider_plt_df, aes(x=pos, y=prop, shape=race)) + 
    geom_errorbar(aes(ymin=overall,ymax=prop), 
                  position=position_dodge(dodge_width), width=0) +
    geom_point(size=2, position=position_dodge(dodge_width), fill='white') +
    annotate("text", x=counties$x, y=.73, label=counties$name, size=3, hjust=1) +
    scale_x_continuous(breaks=c(0,2,8,10,12,16,18,24,28,30,32),
                       labels=levels(consider_plt_df$comm)) +
    scale_y_continuous(breaks=seq(0,.5,0.1), limits=c(0,.75)) +
    labs(
        x='Community', y='Proportion', 
        title='Proportion willing to consider community by race' 
    ) + 
...
## Make table with model with race only 
cons_raceonly <- lapply(comm_names, srch_reg, 'cons')
cons_race_tbl <- huxreg(cons_raceonly, 
                        statistics=c(''), error_format='', borders=0)
comm_titles <- str_to_title(gsub('\\.', ' ', comm_names))
cons_race_tbl[1,] <- c('', comm_titles)
cons_race_tbl[seq(2,8,2),1] <- c('Intercept', 'Asian', 'Black', 'Latino')
cons_race_tbl <- cons_race_tbl[c(1,seq(2,8,2)),]
cons_race_tbl <- t(cons_race_tbl)

## Add model with controls
cons_racecont <- lapply(comm_names, srch_reg, 'cons', m2)
cons_cont_tbl <- huxreg(cons_racecont, 
                        statistics=c(''), error_format='', borders=0,
                        coefs=c('(Intercept)', 'dem.raceapi', 'dem.raceblack',
                                'dem.racelatino'))
cons_cont_tbl[seq(2,8,2),1] <- c('Intercept', 'Asian', 'Black', 'Latino')
cons_cont_tbl <- cons_cont_tbl[seq(2,8,2),]

for(i in 1:4) {
    cons_race_tbl <- insert_column(cons_race_tbl,
                                   as.character(cons_cont_tbl[i, ]),
                                   after=i+4)
    i = i + 1
}
insert_row(cons_race_tbl, c('*** p < 0.001; ** p < 0.01; * p < 0.05', 
                            rep('',ncol(cons_race_tbl)-1)), 
           after=nrow(cons_race_tbl))
tex <- gsub('\\{table\\}', '\\{sidewaystable\\}', to_latex(cons_race_tbl))
cat(tex, file='tables/consider.tex')
```

```{r any-other}
cons_vars <- grep('^cons_', names(dcas), value=TRUE)
cons_vars

for(comm in grep('^dk_', names(dcas), value=TRUE)) {
     print(comm)
     out_fmla <- as.formula(paste0('~',comm))
     res <- MIcombine(with(dcassvy, 
         svyby(out_fmla, '~dem.race', svymean)))
     res <- summary(res)
} 
