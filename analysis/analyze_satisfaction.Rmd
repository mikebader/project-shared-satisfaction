...
Plot predicted probabilities by race using `geom_point` and a line extending from the predicted probability for the racial group to the overall predicted probability in the model with all controls other than race. 
```{r plot-consider-analysis-pt3}
ggplot(cons_probs, aes(x=pos, y=prob, shape=race)) + 
    geom_errorbar(aes(ymin=prob_overall,ymax=prob), 
                  position=position_dodge(dodge_width), width=0) +
    geom_point(size=2, position=position_dodge(dodge_width), fill='white') +
...
```
```{r plot-not-consider-pt1}
races <- c('white','asian','black','latino')
cons_overall <- mutate(cons_overall, pos=ypos)
cons_probs_list <- t(sapply(cons_racecont, srch_gather))
cons_probs <- data.frame(prob=matrix(cons_probs_list[,1:4], ncol=1),
                         lci=matrix(cons_probs_list[,5:8], ncol=1),
                         uci=matrix(cons_probs_list[,9:12], ncol=1),
                         race=rep(races,each=11), comm=rep(comm_names,4)) %>%
    left_join(cons_overall, by='comm', suffix=c('','_overall')) %>%
    as_tibble()
```
