...
satisfied_means <- satisfied_means
...
sat_tbl <- huxreg(m1_sat, m2_sat, m3_sat, statistics=c(), align=".", borders=0) 
...
sat_tbl <- sat_tbl %>%
    ...
    insert_row(c('Neighborhood experience','','',''), after=38)
...
sat_tbl <- sat_tbl[c(-2,-3),] %>% add_rows(intercept, after=43) %>%
    ...
    set_all_padding(0) %>%
    set_label('tab:satisfaction')
font_size(sat_tbl) <- 9.5
sat_tbl
cat(to_latex(sat_tbl[c(1:45,50),]), file='tables/satisfaction.tex')
```
...
m1_esat <- MIcombine(with(dcassvy,
        svyglm(extremely_satisfied ~ dem.race, family=binomial)
))
summary(m1_esat)
m2_esat <- MIcombine(with(dcassvy,
        svyglm(extremely_satisfied ~ dem.race + agec + forbornc + manc + 
                kidsc + marriedc +
               educ1c + educ2c + educ3c + educ5c + inc1c + inc3c + inc4c,
            family=quasibinomial)
))        
...
bet_tbl <- huxreg(m1_bet, m2_bet, m3_bet, statistics=c(), borders=0)
intercept <- bet_tbl[2:3,]
bet_tbl[seq(4,(length(regression_labels_bet)+1)*2,2),1] <- regression_labels_bet
bet_tbl <- bet_tbl %>%
    insert_row(c('Race','','',''),
               after=(which(bet_tbl$names=="(Intercept)")+1)) %>%
    insert_row(c('Demographics','','',''),
               after=(which(bet_tbl$names=="Latinx")+2)) %>%
    insert_row(c('Socioeconomic','','',''),
               after=(which(bet_tbl$names=="Married")+3)) %>%
    insert_row(c('Neighborhood experience','','',''), 
               after=(which(bet_tbl$names=="$150,000+")+4))
bet_tbl <- bet_tbl[c(-2,-3),] %>% 
    add_rows(intercept, 
        after=(which(bet_tbl$names=='Extremely satisfied')+1)) %>%
    set_bottom_border(c(1,45),everywhere,1) %>%
    set_italic(c(2,9), everywhere, TRUE) %>%
    set_caption('Estimated coefficients predicting neighborhood improvement') %>%
    set_all_padding(0)
font_size(bet_tbl) <- 9.5
cat(to_latex(bet_tbl[c(1:45, 52),]), file='tables/improvement.tex')
```
...
mbet_tbl <- huxreg(m1_mbet, m2_mbet, m3_mbet, statistics = c())
