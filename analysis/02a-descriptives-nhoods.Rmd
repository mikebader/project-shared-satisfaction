...
## Descriptive Statistics of Multiracial Neighborhood Population
...
kable(head(country.rank, 3),
      caption="Three largest sending countries in Washington, DC area, 2011-2015 ACS (reported by variable name)")
...
```{r nhd-descriptives-educforborn}
## Source file with function to construct variables for DC area
source('dcarea/dcarea_functions.R')
...
### Population Living in Quadrivial Neighborhoods

Report the number of people living in multiracial neighborhoods in 2015. 
```{r nhd-descriptives-quadpop}
(quadpop <- sum(dcarea[dcarea$quad15==TRUE, 'totpop15'], na.rm=TRUE))

```

### Comparison of Multiracial to All Neighborhoods

Construct table comparing characteristics of the population living in DC-area multiracial neighborhoods to the characteristics of all DC-area neighborhoods. Code creates Table 1 in the manuscript. 

```{r nhd-descriptives-comparison}
...
Write file to LaTeX, including additional formatting. 
```{r nhd-descriptives-comparison-latex}
## Write table to file
...
### Location of Multiracial Neighborhoods in DC Area
...
Construct variables measuring multiracial neighborhoods, both those included using the criteria and those excluded for not meeting the criterion that no group be a majority (reported on page 9 of the manuscript). 
```{r nhd-descriptives-inclusion}
...
kable(sort(multi_tbl[,2], decreasing=TRUE),
      caption='Number of multiracial neighborhoods in each jurisdiction')
...
```{r nhd-descriptives-exclusion-white}
...
```{r nhd-descriptives-exclusion-black}
dcarea[!is.na(dcarea$exclmulti) & dcarea$exclmulti==TRUE & dcarea$race.pnhb>50,
       c('county', racevars)]

```

**Excluded for having a Latinx majority:**
```{r nhd-descriptives-exclusion-latino}
