...
ggsave("images/nhood_change.png", plot = chg_plt,
       width=6.75, units='in')
...
ch_tbl <- insert_row(
    ch_tbl, c("Tract fixed effects", rep("X", 4)),after=statrow) %>%
    set_top_border(statrow+1, everywhere, TRUE) %>%
    set_top_border(statrow+2, everywhere, FALSE) %>% 
    set_top_padding(0) %>%
    set_bottom_padding(0)
cat(to_latex(ch_tbl[,c(1,2,4)]), file="tables/change.tex") ## Models 1 & 3
ch_tbl
