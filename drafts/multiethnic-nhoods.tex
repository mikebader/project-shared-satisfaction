...
Estimating the prospects for durable integration of multiracial neighborhoods requires knowing the level of satisfaction among current residents of integrated neighborhoods. All groups are important, but the historical propensity of whites to shun integration warrants special attention to white satisfaction and preferences. Studies of residential preferences come from samples representing entire metropolitan areas. These samples introduce two problems. First, they average over all whites in the metropolitan area. Presently, however, we are not interested in whites generally, but only those whites living in multiracial neighborhoods. These whites are those that either did not want to move or could not move from integrated neighborhoods. 

Second, studies have examined whether increasing shares of individual groups correspond to lower levels of satisfaction or preferences among whites. But multiracial neighborhoods are categorically different because they represent the \emph{combination} of multiple groups. This categorical difference requires that we study residents of all races living in these neighborhoods. 
...
%  Whites were less likely than their neighbors of color to report neighborhood improvement. On the one hand, the difference could reflect racial variation in baseline assessments of neighborhood quality since most agree that they are satisfied now. On the other, it could indicate that whites might sour more quickly on integrated neighborhoods should changes, race-related or not, transpire. Future work, ideally longitudinal in design, would be able to suss out why whites see less improvement in multiracial neighborhoods than their Asian, black, and Latinx neighbors.
...
%Finding satisfaction among white residents of multiracial neighborhoods does not negate previous research that finds white dissatisfaction with integration. It is possible that \emph{multiracial} integration provides a categorically different stimulus compared to biracial integration upon which previous studies relied. If true, then continuous measures of racial composition by different groups will fail to match the categorically distinct stimuli residents perceive. As a result, future research should consider not just percentages but combinations of racial groups that rely on various combinations to test residential satisfaction and preferences. 
