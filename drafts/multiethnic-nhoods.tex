...
To further show the lack of difference between racial groups, I plotted the predicted probability from the model in the left panel of Figure~\ref{fig:satisfaction}. The plotted figures represent the predicted probability of satisfaction and error bounds based on variances from the multiple imputations.\footnote{The probabilities were predicted for the mean value for continuous variables, modal values for categorical variables, and the tract with the median satisfaction for the fixed effect.} The average marginal effects (AMEs) for non-white groups---the average of the predicted percentage differences across the data if only race were changed---were  \satameoneapi\% for Asians, while blacks and Latinxs were \satameoneblack\% and \satameonelatino\% more likely. 

\abouthere{Figure}{fig:satisfaction}

The second column of Table~\ref{tab:satisfaction} further confirmed that satisfaction was an interracial sentiment among residents of multiracial neighborhoods. The estimates were again small and could not be distinguished from a null effect. The right panel of Figure~\ref{fig:satisfaction} shows the predicted probabilities of satisfaction from the model with controls. I estimated the probabilities and standard errors at the mean value of continuous variables and the modal value for dichotomous and categorical variables. 
%In the complete model, only \maxdiffthree\ separated the least satisfied group (now whites) from the most satisfied (still Latinxs). 
...
\centering
\includegraphics[scale=0.3]{../analysis/images/satisfied.png}\footnote{
This is a footnote.
}
\end{sidewaysfigure}
