...
[^internal-robustness]: A full description of these analyses and the results may be viewed on pages \pageref{rob:internal}--\pageref{rob:comparative} of the supplement. \revision{The analysis showed that white and Asian residents of multiracial neighborhoods with the greatest white population losses were slightly less satisfied in neighborhoods with larger white losses; Latinx and Black residents had similar levels of satisfaction regardless of white population change.}
...
[^entropy]: I also analyzed the association between entropy and satisfaction among groups using only the DC area-wide data from the DCAS 2018. The results were consistent with the findings presented in Figure \ref{fig:between}. I also examined differences between residents of multiracial neighborhoods and all DC-area residents based on a combined model, and those results were also consistent with the results presented in Figure \ref{fig:between}. These analyses can be viewed on pages \pageref{rob:entropy}--\pageref{rob:lntotchg-all} of the supplement.
...
\revision{Table} \ref{tab:desctert} \revision{reports the shares of residents of each race who report being satisfied by the tertile of white population change in their neighborhoods. The differences among Black and Latino residents were very similar regardless of what changes had occurred in among the white population in neighborhoods. The percent of Asian residents satisfied living in neighborhoods that expereinced the lowest tertile of white population change were about 13 percentage points lower than residents in the highest tertile. The difference between white residents living in neighborhodos in first tertile compared to the third tertile was about 10 percentage points.}

\input{../analysis/tables/supp-desctert.tex} 

\revision{Despite the lower levels of satisfaction, especially among Asian and white residents, no *interracial* differences in statisfaction emerged in multivariable models.} To examine whether interracial differences in satisfaction were consistent across this distribution of white neighborhood change, I stratified the DCAS 2016 by tertiles and re-estimated Models 2 and 3 from Table 6 of the manuscript. The parameter estimates, standard errors, and AIC values for the models are reported in Table \ref{tab:chgtert}. Model 2 in each of the tertiles includes race as a predictor of satisfaction. No effect exists that can be statistically distinguished from the null in any of the three models. In the two lowest tertiles, the data fit Model 3, which does not include race, than they fit Model 2. The data for the highest tertile fit the model that includes race better than the model that does not, but none of the race coefficients approach statistical significance.[^education]

\input{../analysis/tables/supp-chgtert.tex}

[^education]: \revision{Educational differences in satisfaction caused the substantial differences between the estimated intercept parameters across the three tertiles. Among high school graduates were satisfied living in neighborhoods in the first tertile, 90.5 percent were satisfied with their neighborhoods. This value ranged from 14.5 percentage points higher (compared to those with a bachelor's degree) to *37.3 percentage points higher* (compared to those with an master's degree) than other groups.}

\revision{Another concern might be that an insufficient number of white respondents lived in neighborhoods that lost white residents from 2000 to 2015. The lack of white respondents across the distribution of white change could indicate a problem with selection out of multiracial neighborhoods. Even in the neighborhoods that lost the most whites (i.e., the first tertile), there were still 97 white respondents (out of 357, or 27 percent).}

\revision{Figure \@ref(fig:supp-chgsat) shows further evidence that white respondents lived in neighborhoods throughout the distribution of white population changes. The top panel shows the smoothed satisfaction among whites across neighborhoods with different changes to the white population. It confirms that white residents in neighborhoods that gained whites were more satisfied than those in neighborhoods that lost whites. But, the bottom panel shows that the result does not come from a sparse number of white respondents. The histogram in the bottom panel shows the number of respondents living in neighborhoods with different levels of white change. The vast majority of white respondents lived in neighborhoods near the median white population change, not in neighborhoods that gained white residents. The narrowness of the confidence intervals in the middle of the distribution relative to the extremes further confirms this point.} 
