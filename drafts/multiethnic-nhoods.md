...
Residents of all races were attracted to new suburban homes since 1980, especially in Montgomery and Fairfax Counties that boasted of nationally renowned school systems. Additionally, Montgomery County has, since 1974, mandated that all housing developments larger than 20 units include a set percentage of units that qualify as affordable housing [@urban_institute_expanding_2012].[^fairfax] Figure \ref{fig:map} \label{rev:map}\revision{shows that multiracial neighborhoods in the DC area do not cluster in the city. Neighborhoods expand out not only to the inner-ring suburbs, but even middle- and outer-ring suburbs in the DC area, and } were especially likely to emerge along the hub-and-spoke system of the region's commuter rail lines in these counties [see also, @holloway_racially_2012]. Figure \ref{fig:map} \revision{reveals the value of studying a representative sample of residents living in multiracial neighborhoods given the underrepresentation of suburban neighborhoods in research on multiracial neighborhoods.}
...
The pattern of lower satisfaction among whites held after controlling for other demographic and neighborhood characteristics. Figure \ref{fig:between} shows the predicted value of neighborhood satisfaction for different racial groups from a model estimated separately from the multiracial neighborhood sample and the complete DC-area sample. I calculated the predictions at the mean value of each control variable in the metropolitan-wide (i.e., DCAS 2018) sample.[^MEM] The plot shows a clear difference in satisfaction between white people living in multiracial neighborhoods and white people in the DC area, and it shows small differences for the other racial groups. The share of Asian residents satisfied in multiracial neighborhoods was only 3.5 percentage points lower than the share of satisfied DC-area Asian residents. Almost no difference existed between Black residents of multiracial neighborhoods and Black residents overall. Latino residents of multiracial neighborhoods were *more likely* to be satisfied than Latinos overall after controlling for individual characteristics. 
...
[^entropy]: \revision{I also analyzed the association between entropy and satisfaction among groups using only the DC area-wide data from the DCAS 2018. The results were consistent with the findings presented in Figure} \ref{fig:between}. \revision{I also examined differences between residents of multiracial neighborhoods and all DC-area residents based on a combined model, and those results were also consistent with the results presented in Figure} \ref{fig:between}. \revision{These analyses can be viewed on pages} \pageref{rob:entropy}--\pageref{rob:lntotchg-all} \revision{of the supplement.}
...
[^opportunity-hoarding]: \label{rev:hoarding}\revision{A few examples of opportunity hoarding in the past five years include opposing school district boundary changes for fear of property value declines in one district and shouting down researchers at an event planning for \emph{potential} redistricting plans in another one }[@woolsey_opposition_2019; @peetz_this_2019]\revision{, opposing homeless shelter locations in wealthy and disproportionately white wards of the city on the basis that if "homeless lives matter; the lives of community homeowners matter too" }[@alpert_upper_2016] \revision{, and fiercely opposing zoning changes in single-family neighborhoods }[@koma_valors_2020]. 
...
At the same time, multiracial neighborhoods do not constitute a panacea. Racial integration requires trading off benefits and costs [@bell_silent_2005]. There are good reasons to support further racial integration, but integration itself does not represent an unalloyed good. People of color, and Black people in particular, are already aware of the trade-offs and have been negotiating these trade-offs for decades [@lacy_blue-chip_2007; @lewis-mccoy_inequality_2014; @lewis_race_2003]. Integration promotes a sense of the "real world," but also exposes them to the daily incivilities and danger of racism [@lacy_blue-chip_2007; @spitz_segregated_2015]. \revision{"Habits of whiteness" } [@walton_habits_2021] \revision{can become the default normative behavior that can marginalize residents of color.}\label{rev:habits} Policies that promote racial integration should also attend to the inclusion and access of residents living in those spaces. That shared satisfaction exists in multiracial neighborhoods does not, however, preclude the possibility that bi-racial integration leads to declining satisfaction and animosity. Future research should investigate satisfaction in different types of integrated neighborhoods. \revision{Relatedly, as Greif's }[-@greif_intersection_2015]\revision{ work has demonstrated the importance of intraracial influence of neighborhood change on satisfaction and should be incorporated into future work }[see also @lacy_blue-chip_2007; @woldoff_white_2011].
...
#### Combining DCAS 2016 and DCAS 2018 data for analysis.\label{rob:combined}

In the second analysis, I combined the data from the DCAS 2016 and DCAS 2018 into a single dataset. The two datasets were combined into a single file and re-weighted to be representative of the DC area population based on the 2014-2018 American Community Survey. 

I used a similar model to that which led to the results reported in Figure \ref{fig:between}. To the model shown in Equation \ref{eqn:model}, I added a term that predicted the influence of being in each of the three samples: the DCAS 2016 multiracial neighborhood sample, the DCAS 2016 predominantly Latino sample, and the DCAS 2018 sample representative of the entire DC area. I also included interactions between respondent race and this sample variable. Table \ref{tab:combined} reports the parameter estimates of the model. 

\input{../analysis/tables/supp-combined.tex}

The parameter estimates confirm a difference between white residents in the DC area generally and those who live in multiracial neighborhoods. The odds of a white resident being satisfied in a multiracial neighborhood are only 34 percent as high as the odds that white residents living in the DC area generally are satisfied ($p<0.001$). Despite white residents having lower odds of being satisfied living in multiracial neighborhoods than neighborhoods generally, white residents of multiracial neighborhoods are still more likely to be satisfied in their neighborhoods than not to be satisfied. At the mean 2018 values of other covariates, the combined analysis predicts that 71 percent of white residents of multiracial neighborhoods will be satisfied (compared to 88 percent of whites in the DC area overall).

Consistent with the findings reported in the analyses in the paper, Black and Latinx residents of multiracial neighborhoods were similarly satisfied in multiracial neighborhoods compared to their counterparts elsewhere. Seventy-six percent of Black residents were satisfied in their neighborhoods in both multiracial neighborhoods and in the DC area generally. Among Latinx residents, 73 percent of those living in multiracial neighborhoods were satisfied compared to 75 percent of DC-area Latinx adults generally.[^predom-latinx] The difference between Black and white residents of multiracial neighborhoods was statistically distinguishable from a null effect ($p<0.05$). The difference between Latinx and white residents in multiracial neighborhoods could not be distinguished from the null, but that appears largely due to the sample size because the effect size of the interaction, $\beta=0.952$, is quite large. Asian residents, in contrast, appear to follow a pattern very similar to white residents. 

[^predom-latinx]: Incidentally, the level of satisfaction among Latinx residents of multiracial neighborhoods was similar to that of Latinx residents of predominantly Latino neighborhoods. 

In summary, the results of the model combining the DCAS 2016 and DCAS 2018 samples are consistent with the results presented in Figure \ref{fig:between} of the paper. White residents of multiracial neighborhoods are less satisfied than white residents generally, a pattern shared only with Asian residents. Black and Latino residents of multiracial neighborhoods were similarly satisfied as Black and Latino residents of the DC area generally. 


#### Differences in white population change between multiracial and other neighborhoods{#rob:lntotchg-all}

A third analysis assessed the influence of changing white population on satisfaction among residents of the DC area. Using the same measure that I used above (the logged ratio of the white populations in 2015 to those in 2000), I examined the distribution of changes respondents in the DCAS 2016 experienced compared to those that the DCAS 2018 experienced. Figure @fig:supp-lntotchg-plt plots these two distributions.
...
I also added several sentences to the conclusions on page \pageref{rev:dc-conclusions} arguing for more comparative analysis between metro areas. I found it difficult to say much more with so little data on the experiences of residents living in multiracial neighborhoods. I do note that the experience might be similar to other "new immigration destination" metropolitan areas that have seen a growth in knowledge work and increases in the number of multiracial neighborhoods, drawing on the types of metros where Wright, et al. (2020, *Sociol Race Ethn*) growing numbers of highly diverse neighborhoods. 
...
#### Estimates from analysis pooling both surveys. 
